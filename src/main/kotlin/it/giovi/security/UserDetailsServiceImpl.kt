...
class UserDetailsServiceImpl(private val userRepository: UserRepository) : UserDetailsService {
    ...
    override fun loadUserByUsername(username: String): UserDetails {
        val user: UserEntity = userRepository.findOne(UserSpecifications.userByUsername(username).and(UserSpecifications.userByState(UserStateEnum.ACTIVE)))
                        .orElseThrow(Supplier<RuntimeException> { UserException(UserExceptionReason.USERNAME_NOT_FOUND_FOR_ENABLED_USER) })
        return create(user)
    }

    fun findById(userId: Long): UserEntity {
        return userRepository.findByIdOrNull(userId) ?: throw UserException(UserExceptionReason.USER_ENTITY_NOT_FOUND)
    }
    ...
    fun findByUsername(username: String): UserEntity {
        return userRepository.findOne(UserSpecifications.userByUsername(username).and(UserSpecifications.userByState(UserStateEnum.ACTIVE)))
            .orElseThrow<RuntimeException> { throw UserException(UserExceptionReason.USER_ENTITY_NOT_FOUND) }
    }
}